...
def placeholder_inputs(batch_size, num_point):
    ...
    labels_pl = tf.placeholder(tf.int32, shape=(None, num_point), name='labels')
    smpws_pl = tf.placeholder(tf.float32, shape=(None, num_point), name='smpws')
...
def get_loss(pred, label, smpw):
    """ pred: BxNxC,
        label: BxN,
	smpw: BxN """
    classify_loss = tf.losses.sparse_softmax_cross_entropy(labels=label, logits=pred, weights=smpw, name='classify_loss')

...
def sample_and_group_all(xyz, points, use_xyz=True):
    ...
    new_xyz = tf.zeros((tf.shape(xyz)[0], 1, 3), dtype=tf.float32)
    idx = tf.constant(np.expand_dims(np.array(range(nsample)), axis=0))
    idx = tf.broadcast_to(idx, shape=(batch_size, 1, nsample))
